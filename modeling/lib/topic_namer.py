...
class SubtopicNamingSignature(dspy.Signature):
    ...
    subtopic_name: str = dspy.OutputField(
        desc="A short, specific sub-category name (2-4 words). Must refine the parent, not repeat it. "
             "Example: if parent is 'Cuisine', good subtopics are 'Pâtisserie', 'Cuisine Asiatique', NOT 'Cuisine'."
    )
...
class SubtopicNamer(dspy.Module):
    ...
    def forward(self, parent_name: str, keywords: List[str], comments: List[str], 
                max_comments: int = 5) -> str:
        """
        Generate a name for a sub-topic, given its parent topic name.
        
        Args:
            parent_name: Name of the parent topic (e.g., "Cuisine")
            keywords: List of representative keywords for the sub-topic
            comments: List of example comments from the sub-topic
            max_comments: Maximum number of comments to include
            
        Returns:
            Generated sub-topic name (e.g., "Pâtisserie")
        """
        keywords_str = ", ".join(keywords[:15])
...
def name_subtopic(parent_name: str, keywords: List[str], comments: List[str]) -> str:
    """
    Name a sub-topic with parent context.
    
    Args:
        parent_name: Name of the parent topic
        keywords: Sub-topic keywords
        comments: Sub-topic example comments
    ...
    return _subtopic_namer(parent_name=parent_name, keywords=keywords, comments=comments)
